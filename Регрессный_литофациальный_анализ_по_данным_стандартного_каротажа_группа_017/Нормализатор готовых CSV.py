...
while True:
    ...
    while True:
        try:
            file = str(input('Если хотите выйти из программы, введите "Stop"\nВведите номер скважины: '))
            print()
            if file.lower() == 'stop':
                print('Завершение работы...')
                ex = input('Введите любое значение: ')
                exit()
            df2 = pd.read_csv(file + '.csv', delimiter=',', encoding='UTF-8')
        except Exception: print('Неправильный ввод, либо скважины с таким название не было найдено.')
        else:
    ...
    def NormDS(df):
        ...
        check = 0
        while check == 0:
            nom = float(input('\nВведите номинальный диаметр скважины (0.190 или 0.216): '))
            if nom == DS_nom_1:
                ...
                print('ALARMA! Неправильный номинальный диаметр скважины.\n'
                      f'Вы ввели {nom}. Введите 0.190 либо 0.216 в зависимости от вашей скважины.\n'
    ...
    df2.to_csv('{}.csv'.format(file + '_norm'), index=False)
